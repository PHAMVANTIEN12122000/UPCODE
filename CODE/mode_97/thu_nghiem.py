...
ty_le = accuracy_score(du_doan, Y)

print('du_doan',ty_le * 100, '%')
'''


model = pickle.load(open('model.pkl','rb'))
#print("Du Doan :",  model.predict([[99	,2	,24	,87,	120,	8.9,	9.7,	2,	1,	0]]))
...
def showPredict():
    ...
    if ((float(txttuoi.get()) <= 0) ):
         ...
         x_input = np.nan;
    '''
    gt = round(float(txtgioitinh.get()),0)
    print('a aa', gt)
    if ( (int(gt) != 1 ) or (int(gt) != 2) ):
         messagebox.showerror("lỗi Sai Thông Tin", " Vui Lòng điền đúng giới tính : 1 Là Nam | 2 Là Nữ ")
         print('nos', int(txtgioitinh.get()))
         print('vao dc day ko')
         x_input = np.nan;
    '''
...
def dochinhxac():
    ...
    messagebox.showinfo("Khả năng dự đoán của SVM", "Độ chính xác của phương pháp: " '97.059%') 
